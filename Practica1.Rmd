...
knitr::opts_knit$set(root.dir = "/Users/ferranibanyezprat")
```
...
sea_temperature<- read_excel("Desktop/1rQ/R&SAS/Practica Final/sea_temperature.xlsx", skip = 5) 
sea_temperature$col<-NA
sea_temperature1<-sea_temperature[1:16,] 
sea_temperature2 <- read_excel("Desktop/1rQ/R&SAS/Practica Final/sea_temperature.xlsx", skip = 23)
...
sea.deep1<-sea.deep1[-keep,]
sea.deep2<-sea.deep2[-keep,]
...
#we check the variable class
class(sea.deep1$Month)
class(sea.deep1$Temperature)
class(sea.deep1$Year)
class(sea.deep1$Depth)
...
#we check the variable class
class(sea.deep2$Month)
class(sea.deep2$Temperature)
class(sea.deep2$Year)
class(sea.deep2$Depth)
...
A<-unique(sea.deep1$Year)
A<-as.vector(A)

for(i in A){
print(subset(sea.deep1,Year==i) %>%
...
```{r}
B<-unique(sea.deep2$Year)
B<-as.vector(B)

for(i in B){
print(subset(sea.deep2,Year==i) %>%
  ggplot( aes(x=Depth, y=Temperature, fill=Depth)) +
    geom_boxplot() +
    scale_fill_viridis(discrete = T, alpha=0.6) +
    geom_jitter(color="black", size=0.4, alpha=0.9) +
...
print(stat.table(list(Year=Year),
           list("Average temperature"=mean(Temperature),
                ...
                "Min"=min(Temperature),
                "Max"=max(Temperature),
                "IQR"=quantile(Temperature,0.5)),data=subset(sea.deep1,Depth==i)))
}
...
library(Epi)
  stat.table(list(Year=Year,Depth=Depth),
           list("Average temperature"=mean(Temperature)),data=sea.deep1)
...
write_xlsx(sea.deep, "/Users/ferranibanyezprat/Desktop/1rQ/R&SAS/R/Practica/NUEVO.xlsx")
```
...
plotmeans(sea.deep1$Temperature ~ sea.deep1$Depth, 
              col=8,
...
```{r}
dataplot<-sea.deep1[sea.deep1$Month=="May" & sea.deep1$Depth==0,]
dataplot$Year<-sequence(18,2000,1)
...
```{r}
bp<-brkdn.plot(Temperature~Depth+Year,data=sea.deep1,main="Evolució de les temperatures mitjanes de l'aigua del mar", xlab="Any", ylab="Temperatura mitjana",pch=1:4,lty=1:4,col=1:4, ylim = c(13,20))
